...
from utils.solver.solver_naive_semantic_parsing import NSPSolver
...
def main(args=sys.argv[1:]):
    ...
    parser.add_argument('--nl_vocab', action='store_true', help='whether use nl vocab or cf vocab')
...
word2id = sp_vocab.nl2id if opt.nl_vocab else sp_vocab.cf2id
logger.info("Vocab size for input natural language sentence is: %s" % (len(word2id)))
...
if not opt.testing:
    params = {
        "src_vocab": len(word2id), "tgt_vocab": len(sp_vocab.lf2id),
        "pad_src_idxs": [word2id[PAD]], "pad_tgt_idxs": [sp_vocab.lf2id[PAD]],
...
if not opt.testing:
    ratio = load_embeddings(train_model.src_embed.embed, word2id, opt.device)
...
solver = NSPSolver(train_model, sp_vocab, loss_function, optimizer, exp_path, logger, device=opt.device, nl_vocab=opt.nl_vocab)
if not opt.testing:
