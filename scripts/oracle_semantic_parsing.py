...
from utils.solver.solver_oracle_semantic_parsing import OSPSolver

############################### Arguments parsing and Preparations ##############################

def main(args=sys.argv[1:]):
    parser = argparse.ArgumentParser()
    parser.add_argument('--task', type=str, help='oracle semantic parsing')
    ...
    # special paras
    parser.add_argument('--use_cf', action='store_true', help='whether use (cf, lf) samples during training')
...
if not opt.testing:
    ...
    cf_dataset = train_dataset if opt.use_cf else []
...
solver = OSPSolver(train_model, sp_vocab, loss_function, optimizer, exp_path, logger, device=opt.device)
if not opt.testing:
    logger.info("Training starts at %s" % (time.asctime(time.localtime(time.time()))))
    solver.train_and_decode(train_dataset, cf_dataset, dev_dataset, test_dataset,
