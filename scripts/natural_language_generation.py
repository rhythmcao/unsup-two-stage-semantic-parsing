...
from utils.solver.solver_natural_language_generation import NLGSolver

############################### Arguments parsing and Preparations ##############################

def main(args=sys.argv[1:]):
    parser = argparse.ArgumentParser()
    parser.add_argument('--task', type=str, help='natural language generation')
...
cf2nl_vocab = Vocab(opt.dataset, task="cf2nl")
logger.info("Vocab size for input canonical form is: %s" % (len(cf2nl_vocab.cf2id)))
logger.info("Vocab size for output natural language is: %s" % (len(cf2nl_vocab.nl2id)))
...
if not opt.testing:
    ratio = load_embeddings(train_model.src_embed.embed, cf2nl_vocab.cf2id, opt.device)
    logger.info("%.2f%% canonical form word embeddings from pretrained vectors" % (ratio * 100))
    ratio = load_embeddings(train_model.tgt_embed.embed, cf2nl_vocab.nl2id, opt.device)
    logger.info("%.2f%% natural language word embeddings from pretrained vectors" % (ratio * 100))
else:
...
loss_function = set_loss_function(ignore_index=cf2nl_vocab.nl2id[PAD], reduction=opt.reduction)
optimizer = set_optimizer(train_model, lr=opt.lr, l2=opt.l2, max_norm=opt.max_norm)

###################################### Training and Decoding #######################################

solver = NLGSolver(train_model, cf2nl_vocab, loss_function, optimizer, exp_path, logger, device=opt.device)
if not opt.testing:
