...
from utils.hyperparam import hyperparam_pretrain_nl2cf_and_cf2nl
...
from utils.solver.solver_pretrain_nl2cf_and_cf2nl import PretrainSolver

############################### Arguments parsing and Preparations ##############################

def main(args=sys.argv[1:]):
    parser = argparse.ArgumentParser()
    parser.add_argument('--task', type=str, help='naive semantic parsing')
    ...
    # special paras
    parser.add_argument('--nl_labeled', type=float, default=1.0, help='ratio of unlabeled natural language utterances we used')
    parser.add_argument('--shared_encoder', action='store_true', help='whether share the encoder for NL2CF and CF2NL models')
    parser.add_argument('--noisy_channel', type=str, default='drop+add+shuffle', help='whether add noise for input')
...
exp_path = opt.read_model_path if opt.testing else hyperparam_pretrain_nl2cf_and_cf2nl(opt)
...
assert len(nl2cf_vocab.nl2id) == len(cf2nl_vocab.cf2id)
...
Example.set_domain(opt.dataset, drop=('drop' in opt.noisy_channel), add=('add' in opt.noisy_channel), shuffle=('shuffle' in opt.noisy_channel))
if not opt.testing:
    train_dataset, dev_dataset = Example.load_dataset(choice='train')
    unlabeled_cf = unlabeled_nl = train_dataset
...
if not opt.testing:
    nl2cf_params = {
        ...
        "shared_encoder": opt.shared_encoder, "noisy_channel": opt.noisy_channel
    }
...
solver = PretrainSolver(nl2cf_model, cf2nl_model, sp_model, vocab, loss_function, optimizer, exp_path, logger, device)
if not opt.testing:
