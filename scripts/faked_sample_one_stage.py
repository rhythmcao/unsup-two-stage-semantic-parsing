...
from utils.solver.solver_faked_sample_one_stage import FSSPSolver

############################### Arguments parsing and Preparations ##############################

def main(args=sys.argv[1:]):
    parser = argparse.ArgumentParser()
    parser.add_argument('--task', type=str, help='faked sample semantic parsing')
    ...
    parser.add_argument('--method', choices=['wmd', 'bow'], default='wmd', help='faked sample method')
...
Example.set_domain(opt.dataset, False, True, False)
if not opt.testing:
...
if not opt.testing:
    params = {
        ...
        "num_layers": opt.num_layers, "cell": opt.cell, "dropout": opt.dropout, "init": opt.init_weight, "method": opt.method
    }
...
if not opt.testing:
    ratio = load_embeddings(train_model.src_embed.embed, sp_vocab.nl2id, opt.device)
...
solver = FSSPSolver(train_model, sp_vocab, loss_function, optimizer, exp_path, logger, device=opt.device, method=params['method'])
if not opt.testing:
