...
model = construct_model['semantic_parsing'](**vars(args)).to(device)

############# model initialization #############
if not args.read_model_path and args.pretrained_embed == 'glove': # init word embeddings with GloVe
...
if not args.testing:
    loss_function = nn.NLLLoss(ignore_index=Example.vocab.lf2id[PAD], reduction='sum')
    num_training_steps = args.max_epoch * ((len(train_dataset) + args.batch_size - 1) // args.batch_size)
    num_warmup_steps = int(args.warmup_ratio * num_training_steps)
    optimizer, scheduler = set_optimizer(model, args, num_warmup_steps=num_warmup_steps, num_training_steps=num_training_steps, module=model.src_embed, name='plm')
    ...
    for i in range(args.max_epoch):
        ...
        for j in range(0, len(train_dataset), args.batch_size):
            ...
            inputs, lens, outputs, out_lens = get_minibatch(train_dataset, task='semantic_parsing', data_index=train_data_index,
                index=j, batch_size=args.batch_size, device=device, input_side=args.train_input_side, pretrained_embed=args.pretrained_embed)
            batch_outputs = model(inputs, lens, outputs[:, :-1])
            loss = loss_function(batch_outputs.contiguous().view(-1, len(Example.vocab.lf2id)), outputs[:, 1:].contiguous().view(-1))
        ...
        dev_acc = decode(model, dev_dataset, os.path.join(exp_path, f'dev.iter{i:d}'), args.test_batch_size, beam_size=args.beam_size, n_best=args.n_best,
            device=device, input_side=args.eval_input_side, pretrained_embed=args.pretrained_embed, task=decode_task)
        logger.info(f'Dev Evaluation:\tEpoch: {i:d}\tTime: {time.time() - start_time:.2f}s\tDev Acc: {dev_acc:.4f}')

        if args.eval_input_side == 'nl': # GEO test dataset does not contain canonical utterance
            start_time = time.time()
            test_acc = decode(model, test_dataset, os.path.join(exp_path, f'test.iter{i:d}'), args.test_batch_size, beam_size=args.beam_size, n_best=args.n_best,
                device=device, input_side=args.eval_input_side, pretrained_embed=args.pretrained_embed, task=decode_task)
    ...
    start_time = time.time()
    dev_acc = decode(model, dev_dataset, os.path.join(exp_path, 'dev.eval'), args.test_batch_size, beam_size=args.beam_size, n_best=args.n_best,
        device=device, input_side=args.eval_input_side, pretrained_embed=args.pretrained_embed, task=decode_task)
    logger.info(f'Evaluation cost: {time.time() - start_time:.2f}s\tDev Acc: {dev_acc:.4f}')

    if args.eval_input_side == 'nl':
        start_time = time.time()
        test_acc = decode(model, test_dataset, os.path.join(exp_path, 'test.eval'), args.test_batch_size, beam_size=args.beam_size, n_best=args.n_best,
            device=device, input_side=args.eval_input_side, pretrained_embed=args.pretrained_embed, task=decode_task)
