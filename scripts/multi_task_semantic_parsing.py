...
from utils.solver.solver_multi_task_semantic_parsing import MTSPSolver

############################### Arguments parsing and Preparations ##############################

def main(args=sys.argv[1:]):
    parser = argparse.ArgumentParser()
    parser.add_argument('--task', type=str, help='multi task semantic parsing')
...
sp_vocab = Vocab(opt.dataset, task="multi_task")
...
Example.set_domain(opt.dataset, True, True, True)
if not opt.testing:
...
if not opt.testing:
    sp_params = {
        "src_vocab": len(sp_vocab.nl2id), "tgt_vocab": len(sp_vocab.lf2id),
        "pad_src_idxs": [sp_vocab.nl2id[PAD]], "pad_tgt_idxs": [sp_vocab.lf2id[PAD]],
    ...
    json.dump(sp_params, open(os.path.join(exp_path, 'sp_params.json'), 'w'), indent=4)
    nl_params = {
        "src_vocab": len(sp_vocab.nl2id), "tgt_vocab": len(sp_vocab.cf2id),
        "pad_src_idxs": [sp_vocab.nl2id[PAD]], "pad_tgt_idxs": [sp_vocab.cf2id[PAD]],
        "src_emb_size": opt.emb_size, "tgt_emb_size": opt.emb_size, "hidden_dim": opt.hidden_dim, "trans": opt.trans,
        "num_layers": opt.num_layers, "cell": opt.cell, "dropout": opt.dropout, "init": opt.init_weight
    }
    json.dump(nl_params, open(os.path.join(exp_path, 'nl_params.json'), 'w'), indent=4)
else:
    sp_params = json.load(open(os.path.join(exp_path, 'sp_params.json'), 'r'))
...
if not opt.testing:
    ratio = load_embeddings(sp_model.src_embed.embed, sp_vocab.nl2id, opt.device)
    logger.info("%.2f%% word embeddings from pretrained vectors" % (ratio * 100))
    ratio = load_embeddings(nl_model.tgt_embed.embed, sp_vocab.cf2id, opt.device)
    logger.info("%.2f%% word embeddings from pretrained vectors" % (ratio * 100))
else:
    model_path = os.path.join(opt.read_model_path, 'sp_model.pkl')
    sp_model.load_model(model_path)
    logger.info("Load SP model from path %s" % (model_path))
    model_path = os.path.join(opt.read_model_path, 'nl_model.pkl')
    nl_model.load_model(model_path)
    logger.info("Load NL model from path %s" % (model_path))

nl_model.src_embed = sp_model.src_embed
nl_model.encoder = sp_model.encoder

# set loss function and optimizer
loss_function = {}
loss_function['sp'] = set_loss_function(ignore_index=sp_vocab.lf2id[PAD], reduction=opt.reduction)
loss_function['nl'] = set_loss_function(ignore_index=sp_vocab.cf2id[PAD], reduction=opt.reduction)
optimizer = set_optimizer(sp_model, nl_model, lr=opt.lr, l2=opt.l2, max_norm=opt.max_norm)

###################################### Training and Decoding #######################################

solver = MTSPSolver(sp_model, nl_model, sp_vocab, loss_function, optimizer, exp_path, logger, device=opt.device)
if not opt.testing:
