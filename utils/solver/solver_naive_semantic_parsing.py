...
class NSPSolver():
    ...
    def __init__(self, model, vocab, loss_function, optimizer, exp_path, logger, device=None, nl_vocab=False, **kargs):
        super(NSPSolver, self).__init__()
        self.model = model
        self.vocab = vocab
        ...
        with open(output_path, 'w') as of:
            for j in range(0, nsentences, test_batchSize):
                ...
                inputs, lens, outputs, _, (raw_cf_inputs, raw_outputs, variables) = get_minibatch(
                    ...
                    index=j, batch_size=test_batchSize, device=self.device, cf_vocab=(not self.nl_vocab), cf_input=True)
                ############################ Forward Model ############################
                with torch.no_grad():
                ...
                inputs, lens, outputs, _, (raw_nl_inputs, _, _) = get_minibatch(
                    data_inputs, self.vocab, task='semantic_parsing', data_index=data_index,
                    index=j, batch_size=test_batchSize, device=self.device, cf_vocab=(not self.nl_vocab), cf_input=False)
                ############################ Forward Model ############################
                with torch.no_grad():
                    results = self.model.decode_batch(inputs, lens, self.vocab.lf2id, beam_size=beam, n_best=n_best)
        ...
        for i in range(max_epoch):
            ...
            for j in range(0, nsentences, batchSize):
                ...
                inputs, lens, outputs, out_lens, _ = get_minibatch(
                    ...
                    index=j, batch_size=batchSize, device=self.device, cf_vocab=(not self.nl_vocab), cf_input=True)
                ############################ Forward Model ############################
