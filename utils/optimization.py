...
def set_optimizer_adadelta(model, args):
    ...
    optimizer = ClippedAdadelta(grouped_params, lr=args.lr, max_norm=args.max_norm)
    return optimizer


def set_optimizer(model, args, num_warmup_steps, num_training_steps, last_epoch=-1, module=None, name='plm'):
    plm = hasattr(module, name) if module is not None else hasattr(model, name)
    if plm and args.layerwise_decay <= 0.: # fix plm params
        for p in getattr(module, name).parameters():
            p.requires_grad = False
    params = [(n, p) for n, p in model.named_parameters() if p.requires_grad]
    no_decay = ['bias', 'LayerNorm.weight']
    # layerwise_decay <= 0., PLM params are not updated ; 
    # 0 < layerwise_decay < 0.5, denotes lr multiplier coefficient for PLM params ;
    # 0.5 < layerwise_decay < 1, lr layerwise decay rate for PLM params ;
    # layerwise_decay >= 1.0, fine-tune the entire model including PLM params as usual
    if plm and 0. < args.layerwise_decay <= 0.5: # seperate lr for plm
        grouped_params = [
            {'params': [p for n, p in params if name in n and not any(nd in n for nd in no_decay)], 'lr': args.layerwise_decay * args.lr, 'weight_decay': args.l2},
            {'params': [p for n, p in params if name in n and any(nd in n for nd in no_decay)], 'lr': args.layerwise_decay * args.lr, 'weight_decay': 0.0},
            {'params': [p for n, p in params if name not in n and not any(nd in n for nd in no_decay)], 'weight_decay': args.l2},
            {'params': [p for n, p in params if name not in n and any(nd in n for nd in no_decay)], 'weight_decay': 0.0},
        ]
        ...
        num_layers = int(getattr(module, name).config.num_hidden_layers)
